...
local settings = {
	audio_api = "pulse", -- use --ao=help to get more
	key_cycle_forward = "a",
	key_cycle_backward = "ctrl+a"
}
	...
	while s ~= e + d do -- until the loop would cycle back to the number we started on
		if string.find(mp.get_property("audio-device"), deviceList[s].name, 1, true) then
			while true do
				...
				s = s + d --next device
				if string.find(deviceList[s].name, settings.audio_api, 1, true) then
...
mp.add_key_binding(settings.key_cycle_forward, "cycle_adevice", cycle_forward)
mp.add_key_binding(settings.key_cycle_backward, "cycleBack_adevice", cycle_back)